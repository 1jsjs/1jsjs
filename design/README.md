...
- Type: Avenir Next Bold for English display, Regular for support; Apple SD Gothic Neo Bold and Regular for Korean. Artwork text is outlined so it does not rely on installed fonts or remote font loading. Body text uses GitHub's native typography.
...
To regenerate on a Mac with Avenir Next and Apple SD Gothic Neo installed: install `fonttools` in a Python environment and run `python design/generate_artwork.py`. Do not commit font binaries.

Language variants are generated together. `README.md` uses English artwork; `README.ko.md` uses the `-ko.svg` files, including both hero themes. Product illustrations, translated visible text, and accessibility descriptions stay in sync in `generate_artwork.py`. Proper technology names and the `jinsu.build` handle are preserved.
